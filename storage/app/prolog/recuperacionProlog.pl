...
% Permite usar assertz/retract sobre estos predicados (personajes creados en runtime)
:- dynamic personaje/3.
:- dynamic inventario/2.

% Carga personajes/inventarios creados dinámicamente desde un archivo aparte
% (este archivo se crea vacío si no existe, ver instrucciones de Laravel)
:- catch(consult('jugadores_extra.pl'), _, true).
...
%taller
% fuerza_total ahora considera tanto inventario/2 base como posee/2 (armas equipadas en runtime)
...
% ===================== NUEVAS REGLAS - Creacion de personajes =====================

% Verifica si ya existe un personaje con ese nombre
existe_personaje(Nombre):-
    personaje(Nombre, _, _).

% Crea un personaje nuevo con nivel y vida dados (si el nombre no existe)
% No se asigna inventario/2 base; el inventario dinamico se maneja con posee/2
crear_personaje(Nombre, Nivel, Vida):-
    \+ existe_personaje(Nombre),
    assertz(personaje(Nombre, Nivel, Vida)).

% Crea el personaje y lo guarda en disco (jugadores_extra.pl) para que persista
% entre ejecuciones del proceso swipl
...
% ===================== NUEVAS REGLAS - Manejo de armas / inventario dinamico =====================

% posee/2 almacena armas asignadas en tiempo de ejecucion (ademas del inventario/2 base)
:- dynamic posee/2.

% Inventario completo de un personaje: une inventario/2 base (si existe) con lo que posee/2 (extra)
% findall+forall evita duplicar logica si inventario/2 no esta definido para el personaje
inventario_completo(Personaje, ListaCompleta):-
    ( inventario(Personaje, Base) -> true ; Base = [] ),
    findall(Arma, posee(Personaje, Arma), Extra),
    append(Base, Extra, ListaCompleta).

% Verifica que el arma exista en el catalogo de fuerzas (weaponforce/2)
arma_valida(Arma):-
    weaponforce(Arma, _).

% Equipa un arma a un personaje existente, si el arma es valida.
% Se guarda en memoria (assertz) y en disco (jugadores_extra.pl) para persistir.
equipar_arma(Personaje, Arma):-
...
% Quita (una ocurrencia) de un arma del inventario dinamico del personaje.
% Solo afecta posee/2 (no modifica inventario/2 base ni reescribe el archivo).
desequipar_arma(Personaje, Arma):-
    retract(posee(Personaje, Arma)).

% fuerza_total ya incluye base + dinamico (ver definicion arriba en seccion de armas)

% Reporte legible del inventario completo y fuerza total de un personaje
reporte_inventario(Personaje, Mensaje):-
    existe_personaje(Personaje),
    inventario_completo(Personaje, Lista),
    fuerza_total(Personaje, Fuerza),
    ( Lista == []
    -> atomic_list_concat([Personaje, 'no tiene armas equipadas. Fuerza total:', Fuerza], ' ', Mensaje)
    ;  atomic_list_concat(Lista, ', ', ListaTexto),
       atomic_list_concat([Personaje, 'tiene:', ListaTexto, '- Fuerza total:', Fuerza], ' ', Mensaje)
    ).
